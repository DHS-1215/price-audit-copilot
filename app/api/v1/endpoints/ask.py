...
# 第二周产出的异常明细文件路径
# 第四周很多数据分析、解释都要基于它
ANALYZED_CSV_PATH = PROJECT_ROOT / "data" / "audit_results_preview.csv"
...
def load_analyzed_df() -> pd.DataFrame:
    """
    读取第二周已经产出的“audit_results_preview.csv”。
    ...
    if not ANALYZED_CSV_PATH.exists():
        raise FileNotFoundError(
            f"未找到异常结果文件：{ANALYZED_CSV_PATH}，请先运行第二周分析流程生成“audit_results_preview.csv”。"
        )
