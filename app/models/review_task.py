...
class ReviewTask(Base, TimestampMixin):
    ...
    task_status: Mapped[str] = mapped_column(
        ...
        nullable=False,
        default="pending",
        comment="任务状态，如 pending / processing / done",
    )
    priority: Mapped[str | None] = mapped_column(
        String(16), nullable=True, comment="优先级"
    )
    assigned_to: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="分配对象"
    )
    ...
    audit_result: Mapped["AuditResult"] = relationship(back_populates="review_tasks")
