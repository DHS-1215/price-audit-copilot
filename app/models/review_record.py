...
class ReviewRecord(Base, CreatedAtMixin):
    ...
    action_type: Mapped[str] = mapped_column(
        String(32),
        nullable=False,
        comment="动作类型，如 confirm_abnormal / mark_false_positive / add_remark",
    )
    action_result: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="动作结果"
    )
    reviewer: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="复核人"
    )
    remark: Mapped[str | None] = mapped_column(
        String(1000), nullable=True, comment="备注"
    )
    evidence_snapshot_json: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, nullable=True, comment="操作时依据快照"
    )

    review_task: Mapped["ReviewTask"] = relationship(back_populates="review_records")
